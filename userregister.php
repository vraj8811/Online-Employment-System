...
<body>
    ...
    <div class="container">
        <?php
        if(isset($_COOKIE['erroruserreg'])) {
            if($_COOKIE['erroruserreg'] == 1) {
                ...
                <div class="signup-form">
                    <form method="post" action="userreg.php">
                        <div class="mb-3">
                            ...
                            <input name="username" class="form-control shadow-sm p-1 mb-2 bg-body rounded"
                                pattern="^(?=[a-zA-Z0-9._]{8,20}$)(?!.*[_.]{2})[^_.].*[^_.]$" type="text" required />

                        </div>
                        <div class="mb-3">
                            <label for="userfullname" required>Full Name :- </label>
                            <input name="userfullname" class="form-control shadow-sm p-1 mb-2 bg-body rounded"
                                pattern="^+{8,32}\S{1,}" type="text" required />

                        </div><div class="mb-3">
                            <label for="userpassword" required>Password :- </label>
                            <input name="userpassword" class="form-control shadow-sm p-1 mb-2 bg-body rounded"
                                pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$"
                                type="password" required />

                        </div><div class="mb-3">
                            <label for="useremail" required>E-mail :- </label>
                            <input name="useremail" class="form-control shadow-sm p-1 mb-2 bg-body rounded"
                                type="email" required />
                        </div><div class="mb-3">
                            <label for="usercity" required>Home City :- </label>
                            <input name="usercity" class="form-control shadow-sm p-1 mb-2 bg-body rounded"
                                type="text" required />
